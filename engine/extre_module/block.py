...
if __name__ == '__main__':   
    module_ = partial("", e=1.0)     
    module = MetaFormer_Mona(in_channel, out_channel, token_mixer=module_).to(device) 

  
class ELFEM(nn.Module):

    def __init__(self, c1, c2, n=1, scale=0.5, e=0.5):
        super(ELFEM, self).__init__()
...
class LSEN(nn.Module):
    def __init__(self,in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        self.fc1 = nn.Conv2d(in_features, hidden_features, 1)
        self.act = act_layer()
        self.fg = layer(hidden_features//2)
        self.c1 = nn.Conv2d(hidden_features,hidden_features//2,1,1)
